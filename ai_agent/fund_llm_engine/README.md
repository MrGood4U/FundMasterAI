...
发 HTTP 请求。基金分析会按数据可用性和基金类型使用后端工具，包括：
...
POST http://127.0.0.1:5003/api/ai/portfolio/analyze
POST http://127.0.0.1:5003/api/ai/sector/analyze
POST http://127.0.0.1:5003/api/ai/news/summary
```
...
当前采用 `mock-first` 回归方式保证流程、契约和编排稳定，同时已经支持真实
OpenAI-compatible 模型。mock 是默认测试路径，不代表真实模型链路尚未实现。
...
- 真实 Git tag 与历史里程碑说明：`docs/version_history.md`
...
`docs/version_history.md` 只用于真实 Git tag 和历史能力线索，不作为当前状态来源。
...
5. mock 与真实模型共用同一套确定性输入、评分、路由和输出契约
