...
This document is the task-level status ledger for the integrated AI Agent
module. It is maintained across the shared development line instead of being
owned by one old feature branch. Use it before reading proposal, planning, or
report documents, then confirm important claims against code and tests.
...
- The integrated Market backend now retrieves Eastmoney stock and bond holdings
  ...
  still-missing duration, maturity, issuer, and credit-rating fields.
...
- The LLM client handles `finish_reason=length`, empty `content`,
  provider-specific `reasoning_content`, retry diagnostics, transient transport
  retries, explicit mock detection, and sanitized HTTP provider errors. Future
  hardening is tracked in `implementation_plan.md` instead of an old phase list.
- `MarketAgent` is implemented using peer percentile and holding-period profit
  evidence. A separate `CapitalFlowAgent` is not registered because the Agent
  layer does not yet have a validated fund-level flow contract.
...
- CapitalFlowAgent remains a data-contract-dependent future extension. The
  frontend can show ETF/order-size flow, but the Agent still needs an agreed
  source, universe, window, units, freshness, and fund mapping before flow can
  affect a fund rating. It is intentionally not registered as a permanently
  skipped Agent.
...
- `CapitalFlowAgent` depends on a validated upstream fund-level flow contract;
  the implemented `MarketAgent` already uses peer/profitability context.
...
```bash
cd ai_agent/fund_llm_engine
.venv/bin/python -m unittest discover -s tests  # 262 passed on 2026-07-15
...
- Treat `version_history.md` as real-tag guidance and conceptual chronology, not
  a complete current-state ledger.
- Runtime secrets, including LLM API keys, must stay runtime-only and must not be written into repository files.
- Update this ledger on the branch that changes Agent behavior, then merge the
  documentation with that behavior. Do not preserve an old feature branch name
  as though it were the permanent source of truth.
